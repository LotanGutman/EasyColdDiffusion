...
class InteractiveAgingTool:
    ...
    def setup_ui(self):
        ...
        # Checkboxes for degradation types
        chk_paper = tk.Checkbutton(control_frame, text="Apply Paper Texture", variable=self.use_paper,
                                   command=self.update_view, bg="#2b2b2b", fg="white", selectcolor="#444444",
                                   font=("Arial", 10))
        chk_paper.grid(row=0, column=1, sticky="w", padx=10)

        chk_scratches = tk.Checkbutton(control_frame, text="Apply Scratches", variable=self.use_scratches,
                                       command=self.update_view, bg="#2b2b2b", fg="white", selectcolor="#444444",
                                       font=("Arial", 10))
        chk_scratches.grid(row=1, column=1, sticky="w", padx=10)

        # Slider for timestep t
        self.slider = tk.Scale(self.root, from_=0, to=MAX_T, orient=tk.HORIZONTAL,
                               command=lambda val: self.update_view(), length=600,
                               label="Degradation Step (t)", bg="#2b2b2b", fg="white",
                               highlightthickness=0, font=("Arial", 12))
        self.slider.pack(pady=10)

        # Image Display Label
        self.lbl_img = tk.Label(self.root, text="Load an image to start...", bg="gray", fg="white", font=("Arial", 14))
        self.lbl_img.pack(pady=20, expand=True)

    def load_texture(self, path):
        """Loads and prepares a texture image supporting Hebrew paths."""
    ...
    def load_image(self):
        ...
        # Pick consistent textures for this specific image
        if self.valid_papers and self.valid_scratches:
            ...
            print("Error: Textures missing from 'textures' folder!")
            return

        self.slider.set(0)
        self.update_view()

    def single_degradation_step(self, img_float):
        """
        ...
        if self.use_scratches.get() and self.scratch_texture is not None:
            ...
            # --- NEW: Morphological Dilation ---
            # Creates a tiny 2x2 kernel to slightly smear the bright pixels outward
            kernel = np.ones((2, 2), np.float32)
            # Dilate the scratch mask (widens the white areas)
            scratch_clean = cv2.dilate(scratch_clean, kernel, iterations=1)
            # We need to restore the channel dimension after dilation (from HxW to HxWx1)
    ...
    def show_img(self, img_bgr):
        """Displays the image in the GUI."""
        img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
        # Upscale for better viewing in the GUI without affecting actual data
